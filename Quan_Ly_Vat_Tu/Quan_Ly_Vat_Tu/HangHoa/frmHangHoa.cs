...
namespace Quan_Ly_Vat_Tu.HangHoa
{
    public partial class frmHangHoa : Form
    {
        ...
        private void bbiSua_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (ma == null)
                return;
            frmHangHoa_Them_Sua frm = new frmHangHoa_Them_Sua(Convert.ToInt64(ma));
            frm.Reload += frm_Reload;
            frm.ShowDialog();
        }
        ...
        private void bbiXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                long[] hanghoa = new long[selectedRows.Length];
                for (int i = selectedRows.Length; i > 0; i--)
                {
                    var arg = gbList.GetRowCellValue(selectedRows[i - 1], colId);
                    if (arg == null)
                        continue;
                    hanghoa[i - 1] = Convert.ToInt64(arg);
                }

                var hh = from h in db.HANG_HOAs
                         where hanghoa.Contains(h.Id)
                         select h;
